...
login_information = {
    'session_key': 'xxxxxxxxx',
    'session_password': 'xxxxxxxx',
    'loginCsrfParam': csrf,
}
...
#input_fields = input_fields.loc[3:,:]

names_b = list(input_fields["linkedin"].values)

ticks = list(input_fields["code_or_ticker"].values)

#names_b = ["SWRG"]
#ticks = ["SWRG"]

for single_coy, ticks in zip(names_b,ticks):
    single_l = [single_coy]
    if lad==4:
        print(single_l)
        rest_list = single_l
        ...
        strong = pages('strong')
        rest_list = [unicodedata.normalize('NFKD', res.contents[0]).encode('ascii', 'ignore').strip().decode('utf-8') for
                     res in strong if res.contents and type(res.contents[0]) == bs4.element.NavigableString]
    print (rest_list)
    ...
    for key, value in rest_id_list.items():
        ...
        response = requests.get('https://www.linkedin.com/voyager/api/organization/companies', headers=headers,
                                params=params)
    ...
    for key, value in com_details_dict.items():
        a = dict((b, c) for b, c in value.items() if
                 b in ['staffCount', 'description', 'industries', 'companyPageUrl', 'jobSearchPageUrl', 'universalName'])
    ...
    for key, value in rest_id_list.items():
        ...
        for i in range(0, 1000, 10):
            ...
            url = 'https://www.linkedin.com/voyager/api/search/cluster?count=10&guides=List(v-%3EPEOPLE,facetCurrentCompany-%3E' + str(
                value) + ')&origin=OTHER&q=guided&start=' + str(i)
            response = requests.get(url, headers=headers)
    ...
    for key, value in emp_details_dict1.items():
        ...
        for val in value:
            if val.get('headless') and val['headless']:
                emp_list.append(dict([(b, c) for b, c in val.items() if
                                      b in ['firstName', 'lastName', 'occupation', 'location', 'industry', 'schools',
                                            'companies']]))
                continue
            response = requests.get(
                'https://www.linkedin.com/voyager/api/identity/profiles/' + val['publicIdentifier'] + '/profileView',
                headers=headers)
            ...
            coms = [dict([(b, c) for b, c in com.items() if b in ['companyName', 'title', 'joining', 'lwd']]) for com in
                    coms]
            val['companies'] = coms
            ...
            schools = [dict([(b, c) for b, c in school.items() if b in ['fieldOfStudy', 'schoolName', 'joining', 'lwd']])
    ...
    for key, value in rest_id_list.items():
        ...
        for i in range(0, 1000, 25):
            print('getting job for {}: {}'.format(key, i))
            ...
            if search_key == 'elements':
                ...
                for b in a[search_key]:
                    ...
                    job = {}
                    print(b['hitInfo'])
                    job['title'] = b['hitInfo']['com.linkedin.voyager.search.SearchJobJserp']['jobPostingResolutionResult'][
                        'title']
                    job['formattedLocation'] = \
                    b['hitInfo']['com.linkedin.voyager.search.SearchJobJserp']['jobPostingResolutionResult'][
                        'formattedLocation']
                    job['companyApplyUrl'] = \
                    b['hitInfo']['com.linkedin.voyager.search.SearchJobJserp']['jobPostingResolutionResult']['applyMethod'][
                        'com.linkedin.voyager.jobs.OffsiteApply']['companyApplyUrl']
                    job['descriptionSnippet'] = b['hitInfo']['com.linkedin.voyager.search.SearchJobJserp'][
                        'descriptionSnippet']
                    job['listedAt'] = str(datetime.datetime.fromtimestamp(
                        b['hitInfo']['com.linkedin.voyager.search.SearchJobJserp']['jobPostingResolutionResult'][
                            'listedAt'] / 1e3))
                    job['expireAt'] = str(datetime.datetime.fromtimestamp(
                        b['hitInfo']['com.linkedin.voyager.search.SearchJobJserp']['jobPostingResolutionResult'][
                            'expireAt'] / 1e3))
        ...
        for k, v in groupby(lst, key=lambda x: x['urn']):
            ...
            for dct in v:
                d.update(dct)
            d = dict((b, c) for b, c in d.items() if
                     b in ['title', 'formattedLocation', 'companyApplyUrl', 'descriptionSnippet', 'listedAt', 'expireAt'])
            list_c.append(d)
    ...
    for key, value in rest_id_list.items():
        ...
        params = (
            ('decoration',
             '(alumniInsights(alumni*(exitYearMonthOn,profile,exitedPosition,endMonthEstimated)),functionHeadcountInsights(latestHeadcountByFunction(yearMonthOn,totalCount,countByFunction*(function~,functionPercentage,functionCount)),headcountGrowthByFunction*),jobOpeningsInsights(jobOpeningsByFunctions*(yearMonthOn,totalCount,countByFunction*(function~,functionPercentage,functionCount)),jobOpeningsGrowthByFunction*,jobOpeningsGrowthAllFunctions*,jobsOpeningsGrowthUnselectedFunctions*,allFunctions*~),headcountInsights,hiresInsights(hireCounts*,totalHires,seniorHires*(hireYearMonthOn,profile,hiredPosition,startMonthEstimated)),upsell,competitors*~(name,staffCountRange,entityUrn,industries,logo),candidateCompetitors*~(name,staffCountRange,entityUrn,industries,logo))'),
        )

        response = requests.get(
            'https://www.linkedin.com/voyager/api/organization/premiumInsights/urn%3Ali%3Afs_normalized_company%3A{}'.format(
                value), headers=headers, params=params)
        ...
        try:
            ...
            for data in a:
                if 'employeeCount' in data or 'totalCount' in data or 'allEmployeeHireCount' in data:
                    data['join_key'] = data['yearMonthOn']
                elif 'year' in data and 'month' in data:
                    data['join_key'] = data['$id']

            c = [b for b in a if 'join_key' in b]
            lst = sorted(c, key=lambda x: x['join_key'])

            list_c = []
            for k, v in groupby(lst, key=lambda x: x['join_key']):
                d = {}
                for dct in v:
                    d.update(dct)

                d = dict((b, c) for b, c in d.items() if
                         b in ['year', 'month', 'day', 'employeeCount', 'totalCount', 'allEmployeeHireCount'])
                list_c.append(d)

            insight['headCountInsight'] = [insight for insight in list_c if 'employeeCount' in insight]
            insight['hireInsight'] = [insight for insight in list_c if 'allEmployeeHireCount' in insight]
            insight['jobOpeningInsight'] = [insight for insight in list_c if 'totalCount' in insight]

            b = {b['localizedName']: b['entityUrn'] for b in a if 'localizedName' in b}
            print(len(b))
            print(b)
            growth_insight_dict = {}
            head_count_insight_dict = {}
            for key1, value1 in b.items():
                fn = [c for c in a if 'function' in c and c['function'] == value1]
                insight_head_count = {}
                insight_job_opening = {}
                for d in fn:
                    if 'functionCount' in d:
                        if 'functionHeadcountInsights' in d['$id']:
                            insight_head_count['functionCount'] = d['functionCount']
                            insight_head_count['functionPercentage'] = d['functionPercentage']
                        else:
                            insight_job_opening['functionCount'] = d['functionCount']
                            insight_job_opening['functionPercentage'] = d['functionPercentage']
                    elif 'growthPeriods' in d:
                        growthPeriods = []
                        for g in d['growthPeriods']:
                            gps = {}
                            gp = [gp for gp in a if '$id' in gp and gp['$id'] == g]
                            gp = gp[0]
                            if 'changePercentage' in gp:
                                gps['changePercentage'] = gp['changePercentage']
                                gps['monthDifference'] = gp['monthDifference']
                            growthPeriods.append(gps)
                        if 'functionHeadcountInsights' in d['$id']:
                            insight_head_count['growthPeriods'] = growthPeriods
                        else:
                            insight_job_opening['growthPeriods'] = growthPeriods
                    if insight_head_count:
                        head_count_insight_dict[key1] = insight_head_count
                    if insight_job_opening:
                        growth_insight_dict[key1] = insight_job_opening
                insight['growthInsight'] = growth_insight_dict
                insight['headCountFunctionInsight'] = head_count_insight_dict
            insight_dict[key] = insight
        except KeyError:
            print("Exception")
            skip = 1

    skip2 = 0
    employee_insights_dict = {}
    for key, value in rest_id_list.items():
        try:
            employee_insight = {}
            params = (
                ('decoration',
                 '(employeeLocationInsights*,employeeSeniorityInsights*,employeeSkillInsights*,employeeEducationDegreeLevelInsights*)'),
                ('q', 'universalName'),
                ('universalName', value),
            )

            response = requests.get('https://www.linkedin.com/voyager/api/organization/culturalInsights', headers=headers,
                                    params=params)

            a = json.loads(response.text)
            a = a['included']

            emp_degree_insights = a[0]['employeeEducationDegreeLevelInsights']
            emp_degree = [dict((b, c) for b, c in emp.items() if b in ['count', 'category', 'percentage']) for emp in a if
                          emp['$id'] in emp_degree_insights]

            emp_location_insights = a[0]['employeeLocationInsights']
            emp_location = [dict((b, c) for b, c in emp.items() if b in ['count', 'category', 'percentage']) for emp in a if
                            emp['$id'] in emp_location_insights]

            emp_seniority_insights = a[0]['employeeSeniorityInsights']
            emp_seniority = [dict((b, c) for b, c in emp.items() if b in ['count', 'category', 'percentage']) for emp in a
                             if emp['$id'] in emp_seniority_insights]

            emp_skill_insights = a[0]['employeeSkillInsights']
            emp_skill = [dict((b, c) for b, c in emp.items() if b in ['count', 'category', 'percentage']) for emp in a if
                         emp['$id'] in emp_skill_insights]

            employee_insight['emp_degree'] = emp_degree
            employee_insight['emp_location'] = emp_location
            employee_insight['emp_seniority'] = emp_seniority
            employee_insight['emp_skill'] = emp_skill

            employee_insights_dict[key] = employee_insight
        except Exception as e:
            print("could not get employee insights for {}".format(key))
    ...
    for key, value in rest_id_list.items():
        ...
        final_dict['insights'] = insight_dict.get(key, None)
        final_dict['employee_insights'] = employee_insights_dict.get(key, None)
    ...
    with open(path_out + ticks + "_final_observation.json", 'w') as fd:
