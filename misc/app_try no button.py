...
def display_content(hidden_dict, trigger, past_hidden_output ):
    ...
    if ka==1:
        ...
        if trigger_n  == 3:
            ...
            option_value_bench_location_dd = trigger["bench_location_info"]
            bench_code_dd = trigger["benchmark_code_info"]
            target_location_small_dd = trigger["target_location_info"]
    ...
    else:    # if k=0 meaning it is run for ht first time
...
def dadr(bench_code_dd, city, dap):
    import layout.location_distance as ld
    dap = json.loads(dap)
    target_code = dap["target_code"]
    ...
    target_file_name = target_code_df[target_code_df["all_target_location_small_names"]==city]["all_target_location_file_names"].reset_index(drop=True)[0]

    closest = ld.dict(target_code, bench_code_dd, target_file_name)

    print(closest)

    option_value_bench_code_dd = bench_code_df[bench_code_df["all_target_location_file_names_b"] == closest["name"]]["all_target_location_small_names_b"].reset_index(drop=True)[0]

    print("poenanananannai")#
...
def dadr(bench_location_info, benchmark_code_info, target_location_info, hidden_dict):
    ...
    fa = 0
    print(bench_location_info,  option_value_bench_location_dd)
    print(benchmark_code_info, bench_code_dd)
    print(target_location_info, target_location_small_dd)

    if bench_location_info == option_value_bench_location_dd:
        fa = fa + 1
    if benchmark_code_info == bench_code_dd:
        fa = fa + 1
    if target_location_info == target_location_small_dd:
        fa = fa + 1
    print(fa)
    if fa ==3:
        None  # should probably make this none again, I don't think return none works
    else:
        return json.dumps({"bench_location_info": bench_location_info, "benchmark_code_info": benchmark_code_info,
                          "target_location_info": target_location_info, "num":fa})
