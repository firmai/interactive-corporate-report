...
for key,value in rest_id_list.items():
    ...
    a = [b for b in a if 'join_key' in b]
    lst = sorted(a, key=lambda x:x['join_key'])
    ...
    for k,v in groupby(lst, key=lambda x:x['join_key']):
        ...
        d = dict((b,c) for b, c in d.items() if b in ['year','month','day','employeeCount','totalCount','allEmployeeHireCount'])
        list_c.append(d)
...
with open(path_out + "final_observation.json" , 'w') as fd:
