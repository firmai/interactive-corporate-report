...
def dic(target_code, target_location_add_info,benchmark_code_info,  bench_location_add_info ):
    ...
    target_location_add_info_1 = target_location_add_info.split(" ")[0]
    bench_location_add_info_1 = bench_location_add_info.split(" ")[0]
    targ_new = target_short_name + ": "+ target_location_add_info_1
    bench_new = bench_short_name + ": "+ bench_location_add_info_1


    initial = list(df["target_small_name"].unique())
    df["target_small_name"] = np.where(df.index == target_location_add_info, targ_new, df["target_small_name"])
    df["target_small_name"] = np.where(df.index == bench_location_add_info, bench_new, df["target_small_name"])
    initial.append(targ_new)
    initial.append(bench_new)

    print(initial)
    print(list(df["target_small_name"].unique()))
    ra = -1
    for i in initial:
        ra = ra + 1
    ...
    map_layout =   html.Div([
                    html.Br([]),
                        ...
                        html.Div([dcc.Graph(figure = fig,
                            ...
                            config={'displayModeBar': False}),
                              ...
                              ],style={'margin-top':'0.3cm','padding-top':'0.0cm','height':'80%'}),
                    html.Div([
                        dt.DataTable(
                    ...
                    ],style={'margin-top':'-2cm','height':'60%','padding-bottom':'1cm'}),
