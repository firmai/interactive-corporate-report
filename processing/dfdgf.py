...
innards = html.Div([
        ...
        html.Div([
            ...
            html.Div([html.H6(["Executive Summary"], style={"float": "left", "padding-right": "0.2cm"}),
                      html.A("info",
                             href='http://4d.readthedocs.io/en/latest/text/executive_summary.html#executive-summary',
                             target="_blank")
        ...
        html.Div([
            ...
            html.Div([
                html.H6(["Shareholder Performance"],
                        className="gs-header gs-table-header padded", title="This chart compares the past shareholder financial performance of the target company and the "
        ...
        html.Div([
            ...
            html.Div([
                html.H6('Stakeholder Metrics',
                        className="gs-header gs-text-header padded", title="The Stajeholder Metrics identified the 4 most important stakeholder dimensions and their "
            ...
            html.Div([
                ...
                html.H6("Financial Performance",
                        className="gs-header gs-table-header padded", title="This financial performance table looks at essential finacial metrics in four important years starting with the "
                                                                            "last financial year, Yr 1 and ending with the fifth last financial year, Yr 5. If the company is private this information "
                                                                            "is guissed with an AI prediction tool. This part is still experimental and working to be improved. The benchmark is the company "
    ...
    html.Div([  # subpage 1
        ...
        html.Div(id='loading', children=[
            ...
            html.Div([
                ...
                html.Div([
                html.H5(first_dict["title_output"], id='title',style={"float":"left"}),

                #html.Img(src="//logo-core.clearbit.com/bjsrestaurants.com?size=60", style={"float":"center","padding-left":"2cm","margin-left":"1.5cm","margin-right":"-1.5cm"}),
                    ], id="img_logo"),

                html.H6(first_dict["location_output"], id='location',
                        style={'color': '#7F90AC','margin-top': '-0.6cm'}),
                ...
                ], style={'padding-top': '0.0cm', 'padding-left': '0cm','margin-top': '0.2cm'},
        ...
        html.Div([
            ...
            ], style={'float': 'left','margin-left':"-1cm","margin-top":"-1cm"}, id="img_logo"),
...
other_layout = html.Div([
    ...
    html.Div([  # page 3
        ...
        html.Div([  # subpage 2
            ...
            "website, management, customers, location, news and "
            "employees are used to algorithmically filter the benchmarks down to 7 firms. "
                   "The website algorithm tracks the cookies of users to calculate audience overlap using two independent tools. The management algorithm identifies companies that "
                   "compete for the same cohort of managers by using Linkedin data. The customer algorithm identifies firms that have an overlapping customer base as "
                   "calculated using Facebook and Yelp data. The before mentioned as well as geolocation metrics are then used to identify firms "
                   ...
                   " the employee algorithm looks at firms that compete for a similar lower level employee cohort sample using Glassdoor.  "),
    ...
    html.Div([  # page 1
        ...
        html.Div([  # subpage 1
            ...
            html.Div([html.H6(["Trend Analysis"],
                              className="gs-header gs-table-header padded"),
                      html.P(inp.trends, style={"padding-top": "1mm"}),
                      html.Table(make_dash_table(pd.read_csv("data/ratings/corr_mat.csv"))),
    ...
    html.Div([  # page 5
        ...
        html.Div([  # subpage 2
            ...
            html.Div([
                dcc.Tabs(
                    tabs=[{'label': "Website", 'value': "Website"},
...
def clean_data(bench_code_dd, clicks, target_location_small_dd, riffy):
    ...
    if bench_code_dd != None:
        ...
        if not all((bench_code_dd, target_location_small_dd)):
            ...
            hidden_dict = {'target_short_name': target_short_name, 'target_location_file_name': target_location_file_name,
...
def dadr(hidden_dict):
    ...
    s_metrics_df = pd.DataFrame([["Sentiment", "E", "C", "S", "M", "A", "BA"],
                                 [hidden_dict["target_code"], new_frame[new_frame.index==hidden_dict["target_code"]].round(1)["Employees"].values[0],
                                  new_frame[new_frame.index==hidden_dict["target_code"]].round(1)["Customers"].values[0],
                                  new_frame[new_frame.index==hidden_dict["target_code"]].round(1)["Valuation Part"].values[0],
                                  new_frame[new_frame.index==hidden_dict["target_code"]].round(1)["Management"].values[0],
                                 ...
                                 [ hidden_dict["bench_code_dd"],
                                  new_frame[new_frame.index ==  hidden_dict["bench_code_dd"]].round(1)["Employees"].values[
                                      0],
                                  new_frame[new_frame.index ==  hidden_dict["bench_code_dd"]].round(1)["Customers"].values[
                                      0],
...
def dadr(hidden_dict):
    ...
    lat = html.Div([
        ...
        html.Table(make_dash_table(rad[hidden_dict["target_code"]]), id="competitor_table"),
        ...
        html.Div([
            ...
            html.H6(["Top Four Competitors"]),
            html.P("This section identifies the MSE (Marginal Squared Errors between "
                   "4 components to identify the 4 most simlar firms to the target company.", title="This is the second method to filter down"
                ...
                html.Div([
                    html.Div([
                        dcc.Graph(figure=fg[0], config={'displayModeBar': False}, id='comp_plot',
                                  style={'border': '0', 'width': "50%", 'height':'4%'}),
            ...
            ], style={"margin-left": "1.5cm","margin-bottom": "-1cm","padding":"0cm",'height':'30%'}),
...
def dadr(hidden_dict):
    ...
    listed = input_fields[input_fields["code_or_ticker"]==hidden_dict["target_code"]]["listed"].reset_index(drop=True).values[0]
    ...
    if listed=="No":
        if df2["close"].iloc[-1]>df_com["close"].iloc[-1]:
            desc = desc + " The AI predicted value growth is higher than the industry benchmark."
        elif df2["close"].iloc[-1]<df_com["close"].iloc[-1]:
            desc = desc + "  The AI predicted value growth is lower than the industry benchmark."
...
@app.callback(
    ...
    [dash.dependencies.Input('rating_dd', 'value'),
     dash.dependencies.Input('county_state_dd', 'value')
        , ])
def dadr(rating_dd, county_state_dd):
...
@app.callback(
    ...
    [dash.dependencies.Input('benchmary_dd', 'value'),
     ...
     dash.dependencies.Input('county_state_dd', 'value')
        , ])
def dadr(benchmary_dd, rating_dd, county_state_dd):
    print("DDDDFDFF")
...
@app.callback(
    ...
    [Input('tabs-customer', 'value'),
     ...
     dash.dependencies.Input('target_location_info', 'value')
        , ])
def dadr(value, dar, bench_location_info, benchmark_code_info, target_location_info, ):
    import layout.location_distance as ld
...
def update_datatable(radio, bench_location_info, benchmark_code_info, target_location_info):
    target_code = input_fields[input_fields["starting"] == 1]["code_or_ticker"].reset_index(drop=True)[0]
...
@app.callback(Output('datatable', 'rows'), [Input('tabs-areas', 'value')],
              [dash.dependencies.State('bench_location_info', 'value'),
               dash.dependencies.State('benchmark_code_info', 'value'),
               dash.dependencies.State('target_location_info', 'value'), ])
def update_datatable(second_tab, bench_location_info, benchmark_code_info, target_location_info):
    target_code = input_fields[input_fields["starting"] == 1]["code_or_ticker"].reset_index(drop=True)[0]
