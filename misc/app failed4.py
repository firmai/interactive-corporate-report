...
import layout.input_infograph_layout as iil
import layout.output_infograph_layout as oil
...
app.layout = html.Div([
    ...
    html.Div(id='hidden_output_trigger', style={'display': 'none'}),
    html.Div(id='hidden_output_trigger_info', style={'display': 'none'}),
    html.Div(id='trig_one', style={'display': 'none'}),
    html.Div(id='trig_two', style={'display': 'none'}),
    html.Div(id='trig_three', style={'display': 'none'}),
...
html.Div([  # page 5
        ...
        html.Div([  # subpage 2
            ...
            html.Div([
                ...
                html.Div(id='tab-input-customer'),
...
def dadr(hidden_dict):
    hidden_dict = json.loads(hidden_dict)
    title = str(hidden_dict["target_location_small_dd"]) + " Profile"
...
@app.callback(
    dash.dependencies.Output('tab-input-customer', 'children'),
...
def dadr(value, dar ):
    ...
    print('tab-input-customer', 'children')
    ...
    i = iil.dict(options_value_target_location_small_dd, option_value_bench_code_dd,
...
def dadr(bench_code_dd, city, dap, bench_code_dd_old):
    import layout.location_distance as ld
    dap = json.loads(dap)
    target_code = dap["target_code"]
...
def dadr(bench_code_dd):
    ...
    print('bench_location_info', 'options')
...
app.callback(
    dash.dependencies.Output('hidden_output_trigger', 'children'),
    [dash.dependencies.Input('tab-input-customer', 'children')],
     [dash.dependencies.State('bench_location_info', 'value'),
     dash.dependencies.State('benchmark_code_info', 'value'),
     dash.dependencies.State('target_location_info', 'value'),
     dash.dependencies.State('hidden_output', 'children')])

def dadr(bench_location_info, benchmark_code_info, target_location_info, hidden_dict):
    import layout.location_distance as ld

    print('hidden_output_trigger', 'children')

    hidden_dict = json.loads(hidden_dict)
    target_code = hidden_dict["target_code"]

    bench_code_dd = hidden_dict["bench_code_dd"]
    target_location_small_dd = hidden_dict["options_value_target_location_small_dd"]
    target_short_name = hidden_dict["target_short_name"]
...
@app.callback(
    ...
    [dash.dependencies.Input('hidden_output_trigger', 'children')],
    [dash.dependencies.State('bench_location_info', 'value'),
     dash.dependencies.State('benchmark_code_info', 'value'),
     dash.dependencies.State('target_location_info', 'value'),
     dash.dependencies.State('tabs-customer', 'value'),])

def dadr(trigger, bench_location_info_v,benchmark_code_info_v,  target_location_info_v, value ):
    print('tab-output-customer', 'children')
    ...
    # First art is to get the target location data from the file name.
    target_code = input_fields[input_fields["starting"] == 1]["code_or_ticker"].reset_index(drop=True)[0]

    # Remove this if you want to clean up:
    print("trigger")
    print(trigger)
    try:
        trigger_n = json.loads(trigger)
        trigger_n  = trigger_n["num"]
        if trigger_n  <20:
            print("trigger")
            print(trigger_n )
            ka = 1
    ...
    if ka==1:
        ...
        if trigger_n  == 3:
            ...
            bench_location_info_v = trigger["bench_location_info"]
            benchmark_code_info_v = trigger["benchmark_code_info"]
            target_location_info_v = trigger["target_location_info"]


    options_value_target_location_small_dd = target_location_info_v     # 1
    ...
    target_location_file_name = \
        comp_ad_df[comp_ad_df["all_target_location_small_names"] == options_value_target_location_small_dd][
            "all_target_location_file_names"].reset_index(drop=True)[0]

    # Now for the benchmark file names
    bench_code_dd_ad_df = pd.DataFrame()

    all_target_location_file_names_b = ext_info_dict[benchmark_code_info_v]["All Target Location File Names"]
    all_target_location_small_names_b = ext_info_dict[benchmark_code_info_v]["All Target Location Small Names"]
    all_target_location_full_addresses_b = ext_info_dict[benchmark_code_info_v]["All Target Location Full Addresses"]
    ...
    bench_location_file_name = \
        bench_code_dd_ad_df[bench_code_dd_ad_df["all_target_location_small_names"] == bench_location_info_v][
            "all_target_location_file_names"].reset_index(drop=True)[0]
    ...
    figures_dict_b = pickle.load(open(path_in_ngrams + "figures_dict_" + benchmark_code_info_v + ".p", "rb"))

    fig_b = figures_dict_b[benchmark_code_info_v, bench_location_file_name]
    ...
    fig_b_n=agg[benchmark_code_info_v]


    i = oil.dict(fig_d,fig_b, fig_d_n, fig_b_n,target_code)
...
### I am not 100% convinved but I think it should be states - otherwise chuck them back in input
