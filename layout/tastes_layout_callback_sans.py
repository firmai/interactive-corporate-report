...
#
def di(target, bench, d_target, d_bench, fally):
    ...
    def dic(coy, d_target, loca_frame):
        ...
        long_name = \
        adds[adds["All Target Location Small Names"] == d_target]["All Target Location File Names"].reset_index(
            drop=True)[0]

        rads = adds[adds["All Target Location Small Names"].isin(loca_frame[coy])]["All Target Location File Names"]
        shorties = adds[adds["All Target Location Small Names"].isin(loca_frame[coy])][
            "All Target Location Small Names"]
        addies = adds[adds["All Target Location Small Names"].isin(loca_frame[coy])][
        ...
        das = [{"lat": lat, "lon": lon, "name": name, "short": short, "add": add} for lat, lon, name, short, add in
               zip(list(lf["lat"]), list(lf["lon"]), list(lf["name_list"]), list(lf["short"]), list(lf["adds"]))]
    ...
    tlonc = tlonc.append(pd.Series([str(tronc.mean()["rating"]) + "/5", "$" + str(round(tronc.mean()["prices"], 2))],
                                   index=tlonc.columns), ignore_index=True)
    ...
    lonc = lonc.append(pd.Series([str(bronc.mean()["rating"]) + "/5", "$" + str(round(bronc.mean()["prices"], 2))],
                                 index=lonc.columns), ignore_index=True)
